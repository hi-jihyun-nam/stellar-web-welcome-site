...
Best regards`;
  ...
  return <section className="py-32 px-6 relative bg-gradient-to-b from-purple-50/20 via-gray-50 to-blue-50/30 overflow-hidden">
      {/* Background effects */}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 max-w-6xl mx-auto">
          ...
          <div>
            ...
            <div className="relative p-8 rounded-3xl transition-all duration-500 hover:scale-105 hover:bg-cyan-50/50 shadow-lg bg-white/80 backdrop-blur-xl border border-gray-200/50 hover:border-cyan-300/60 hover:shadow-xl hover:shadow-cyan-200/20">
              ...
              <ul className="space-y-4 mb-8">
                {b2cService.features.map((feature, featureIndex) => <li key={featureIndex} className="flex items-start gap-3">
                    <Check className="w-5 h-5 text-emerald-500 flex-shrink-0 mt-0.5" />
                    <span className="text-gray-600 font-light leading-relaxed">{feature}</span>
                  </li>)}
              </ul>

              <Button onClick={() => handleEmailContact("Complete Care Service")} className="w-full py-3 rounded-xl font-semibold transition-all duration-300 bg-gradient-to-r from-cyan-500 to-cyan-600 hover:from-cyan-600 hover:to-cyan-700 text-white border-0 shadow-lg hover:shadow-xl">
                Begin Journey
              </Button>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="relative p-8 rounded-3xl transition-all duration-500 hover:scale-105 hover:bg-purple-50/50 shadow-lg bg-white/80 backdrop-blur-xl border border-gray-200/50 hover:border-purple-300/60 hover:shadow-xl hover:shadow-purple-200/20">
              ...
              <ul className="space-y-4 mb-8">
                {b2bService.features.map((feature, featureIndex) => <li key={featureIndex} className="flex items-start gap-3">
                    <Check className="w-5 h-5 text-emerald-500 flex-shrink-0 mt-0.5" />
                    <span className="text-gray-600 font-light leading-relaxed">{feature}</span>
                  </li>)}
              </ul>

              <Button onClick={() => handleEmailContact("Strategic Partnership Service")} className="w-full py-3 rounded-xl font-semibold transition-all duration-300 bg-gradient-to-r from-purple-500 to-purple-600 hover:from-purple-600 hover:to-purple-700 text-white border-0 shadow-lg hover:shadow-xl">
                Start Partnership
              </Button>
            </div>
          </div>
